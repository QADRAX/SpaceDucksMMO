...
type ResourceSummary = { id: string; kind: 'basicShaderMaterial'; activeVersion: number };

const DEFAULT_SHADER_CODE = `vec4 fragmentMain(vec2 uv, float time) {
  return vec4(uv.x, uv.y, sin(time), 1.0);
}`;

export function BasicShaderDetailPanel({ resource, versions }: { resource: ResourceSummary; versions: VersionSummary[] }) {
    ...
    React.useEffect(() => {
        if (shaderFile) {
            ...
            reader.onload = (e) => {
                if (typeof e.target?.result === 'string') setShaderSource(e.target.result);
            };
    ...
    const submitNewVersion = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const zipFile = new File([zipBlob], `basic-${resource.id}.zip`, { type: 'application/zip' });
    ...
    return (
        <div className="h-full min-h-0 flex flex-col">
            ...
            <ResourceDialogLayout open={creatingOpen} onOpenChange={setCreatingOpen} onClose={() => setCreatingOpen(false)} title="New Basic Shader Version" fullscreen={true} className="max-w-7xl">
                <div className="flex h-full w-full bg-neutral-900 overflow-hidden">
                    <ResourceDialogFormPanel onSubmit={submitNewVersion} error={error} className="w-[450px] shrink-0 border-r border-neutral-800 bg-neutral-950 flex flex-col overflow-y-auto">
                        <div className="p-4 space-y-4 flex flex-col h-full">
                            ...
                            <div className="flex-1 flex flex-col space-y-2 pb-4">
                                <Label>Shader Source</Label>
                                <textarea className="w-full flex-1 min-h-[300px] p-3 font-mono text-xs bg-[#1e1e1e] text-[#d4d4d4] border border-neutral-800 rounded resize-none focus:outline-none" value={shaderSource} onChange={(e) => setShaderSource(e.target.value)} spellCheck={false} />
                            </div>
                    ...
                    <ResourceDialogPreviewPanel className="flex-1 relative">
                        <BasicShaderPreview resourceKey={`basic-${resource.id}`} shaderSource={shaderSource} uniforms={getFinalUniforms()} componentData={componentData} onError={(err) => setError(err ? err.message : null)} />

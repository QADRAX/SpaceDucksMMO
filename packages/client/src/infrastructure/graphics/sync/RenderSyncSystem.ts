...
export class RenderSyncSystem implements IComponentObserver {
  ...
  private buildGeometry(
    params: GeometryComponent["parameters"]
    ...
    switch (params.type) {
      ...
      case "custom":
        console.warn(
          `[RenderSyncSystem] custom geometry key='${params.key}' not implemented; using unit box`
        );
  ...
  private buildMaterial(comp?: MaterialComponent): THREE.Material | undefined {
    ...
    // textures
    if (comp.texture) {
      this.textureLoader.load(comp.texture, (tex) => {
        (material as any).map = tex;
        material.needsUpdate = true;
      });
    }
    if (comp.normalMap) {
      this.textureLoader.load(comp.normalMap, (tex) => {
        (material as any).normalMap = tex;
        material.needsUpdate = true;
      });
    }
    if (comp.envMap) {
      this.textureLoader.load(comp.envMap, (tex) => {
  ...
  private buildShaderMaterial(
    ...
    for (const [k, u] of Object.entries(comp.uniforms)) {
      if (u.type === "texture") {
        ...
        this.textureLoader.load(u.value, (tex) => {
    ...
    return new THREE.ShaderMaterial({
      ...
      depthWrite: (comp as any).depthWrite,
      blending,
    });
  }
  ...
  private createMesh(
    ...
    if (shaderMaterialComp)
      material = this.buildShaderMaterial(shaderMaterialComp)!;
    else material = this.buildMaterial(materialComp)!;
  ...
  private syncMaterial(entity: Entity): void {
    ...
    if (rc.material) rc.material.dispose();
    const newMat = this.buildMaterial(materialComp)!;

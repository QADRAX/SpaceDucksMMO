    ...
    <nav className={cn('w-72 bg-white border-r-4 border-black p-8', className)}>
      <div className="mb-10">
        <div className="bg-main border-4 border-black p-4 rounded-base shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <p className="text-xs font-bold mt-1 opacity-70">Web Core</p>
        </div>
      </div>
      {children}
    </nav>
  ...
  return (
    <li>
      <Link
        href={href}
        className={cn(
          'block px-5 py-3 rounded-base font-bold text-black border-2 border-black transition-all hover:translate-x-0.5 hover:translate-y-0.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-none active:translate-x-0.75 active:translate-y-0.75 bg-white hover:bg-main',
          className
        )}
      >
        {children}
